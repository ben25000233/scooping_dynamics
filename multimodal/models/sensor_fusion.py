...
class SensorFusion(nn.Module):
    ...
    def forward_encoder(self, hand_depth, ee_pose, front_depth):
        batch_size,image_num, _, _ = hand_depth.shape
        empty_tensor = torch.empty(batch_size, 0, 128).to(self.device)
        
   
        # Get encoded outputs
        # pose_out = self.ee_normalize(ee_pose)
        # front_pcd = self.pcd_encoder(front_pcd)
        # hand_pcd = self.pcd_encoder(hand_pcd)
        ...
        for i in range(image_num):
            ...
            empty_tensor = torch.cat((empty_tensor, hand_depth_out, front_depth_out), 1)
      

        embeddings = torch.cat((pose_out, empty_tensor), 1).to(torch.float32)
...
class Dynamics_model(SensorFusion):
    ...
    def __init__(
        ...
        if training_type == "spillage" : 
            num_type = 5
        else : 
            num_type = 6

        self.fc1 = nn.Linear(56 * 128, 512)  # Flatten the input
        self.relu1 = nn.ReLU()
    ...
    def forward(
        ...
        # letent_z = self.multi_encoder.forward_encoder(hand_depth, hand_seg, ee_pose_in, front_depth)
