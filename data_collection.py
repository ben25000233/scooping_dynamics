...
class IsaacSim():
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(num_envs):
            ...
            bowl_pose.p = gymapi.Vec3(0.53, -0.13 , 0.04)  
            ...
            y = gymapi.Quat.from_axis_angle(gymapi.Vec3(0,1,0), np.radians(57))
            ...
            camera_offset = gymapi.Vec3(0.03, -0.0345, 0.094)
            ...
            # degree 276.8
            camera_rotation = gymapi.Quat.from_axis_angle(gymapi.Vec3(0,1,0), np.radians(276.8))
    ...
    def cal_spillage_scooped(self, env_index, reset = 0):
        ...
        if reset == 0:
            ...
            if (self.dpose_index[env_index]+1) % 72 == 0 : 
                split_spillage = sum(self.spillage_vol[env_index][-6:])
    ...
    def reset_franka(self, franka_idx, init = 0):
        ...
        franka_init_pose = np.append(temp[50], 0.02)
    ...
    def data_collection(self):
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            for i in range(self.num_envs):
                ...
                if self.frame % self.slow_time == 3 and self.frame > 101:
                    self.get_info(i)
    ...
    def get_info(self, env_index):
        ...
        if self.round[env_index] < self.collect_time :
            ...
            self.top_depth_list[env_index].append(self.normalize_image(top_depth_image))
            ...
            plt.imshow(top_color_image)
            plt.show()
            # plt.savefig('sim_seg.png')
            exit()

            self.hand_rgb_list[env_index].append(hand_color_image)
            self.hand_depth_list[env_index].append(self.normalize_image(hand_depth_image))
            self.hand_seg_list[env_index].append(self.normalize_image(hand_seg_image))
