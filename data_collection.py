...
class IsaacSim():
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(num_envs):
            ...
            camera_offset = gymapi.Vec3(0, 0, 0.1)
            camera_rotation = gymapi.Quat(0.6, 0, 0.9, 0)
    ...
    def get_pcd(self, env_index):
        ...
        if self.round[env_index] < (self.collect_time*4) :
            ...
            point_cloud = self.depth_to_point_cloud(env_index, top_depth_image, type = 0)
            self.top_pcd_list[env_index].append(np.array(point_cloud.points))
            

            #o3d.io.write_point_cloud(f"{self.file_root}/top_view/env_{env_index}_round_{self.round[env_index]}.ply", point_cloud)
            ...
            # o3d.io.write_point_cloud(f"{self.file_root}/hand_view/env_{env_index}_round_{self.round[env_index]}.ply", point_cloud)
