...
class pcd_Encoder():
    ...
    def encode(self, pcd):
        ...
        with torch.no_grad(): 
            all_pcd = torch.zeros(batch_size, 0).to(device)
            pcd = pcd.to(device)

            for index in range(pcd.shape[1]):
                single_pcd = pcd[:, index, :, :]
                output_pcd = self.model(single_pcd)
                
                output_pcd = output_pcd.reshape(output_pcd.shape[0], 256)
                all_pcd = torch.cat((all_pcd, output_pcd), dim=1) 
