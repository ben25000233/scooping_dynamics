...
class MultimodalManipulationDataset(Dataset):
    ...
    def _read_data_from_file(self, dataset, idx):
        ...
        if current_index == 0:
            ...
            top_pcd = np.tile(dataset["top_pcd_point"][0], (look_back_frame+1, 1, 1)).astype(np.float32)
            ...
            top_pcd = dataset["top_pcd_point"][begin_idx:end_idx].astype(np.float32)
        current_ee_index = int((idx + 1) * single_num)
        eepose = np.concatenate((all_eepose[current_ee_index], all_eepose[current_ee_index+7]))
        eepose = eepose.reshape(2,7)
