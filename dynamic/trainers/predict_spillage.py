...
class predict_spillage:
    def __init__(self, configs):
        ...
        #     weight_decay=0.02,
        # )
        self.optimizer = optim.SGD(self.model.parameters(), lr=self.configs["lr"], momentum=0.99, weight_decay=0.0)
    ...
    def loss_calc(self, sampled_batched):
        ...
        reference = torch.tensor([1., 0., 0., 0., 0.], device=self.device, dtype=torch.float32)
        ...
        if len(zero_indices) > int(len(non_zero_indices)/4):
            ...
            hand_pcd = hand_pcd[combined_indices]
            front_pcd = front_pcd[combined_indices]
        ...
        pred_spillage = self.model(
                hand_depth, eepose, front_depth,hand_pcd, front_pcd, 
            )
