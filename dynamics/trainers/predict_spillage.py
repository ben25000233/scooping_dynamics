...
class predict_spillage:
    ...
    def loss_calc(self, sampled_batched):
        ...
        hand_seg = sampled_batched["hand_seg"].to(self.device)
        front_seg = sampled_batched["front_seg"].to(self.device)
        ...
        # remove too much zero case(multi-label)

        reference = torch.tensor([1., 0., 0.,], device=self.device, dtype=torch.float32)
        non_zero_indices = torch.where((spillage != reference).any(dim=1))[0]
        zero_indices = torch.where((spillage == reference).all(dim=1))[0]
        if len(zero_indices) > int(len(non_zero_indices)/2):
            selected_indices = zero_indices[torch.randperm(len(zero_indices))[:10]]
            ...
            eepose = eepose[shuffled_indices]
            spillage = spillage[shuffled_indices]

            hand_depth = hand_depth[shuffled_indices]
            front_depth = front_depth[shuffled_indices]

            hand_pcd = hand_pcd[shuffled_indices]
            front_pcd = front_pcd[shuffled_indices]
            ee_pcd = ee_pcd[shuffled_indices]

            hand_seg = hand_seg[shuffled_indices]
            front_seg = front_seg[shuffled_indices]

        
        # remove too much zero case(single-label)

        # indices_ones = torch.where(spillage == 1)[0]
        # indices_zeros = torch.where(spillage == 0)[0]

        # if len(indices_zeros) > len(indices_ones):
        #     if len(indices_ones) == 0:
        #         selected_indices = indices_zeros[torch.randperm(len(indices_zeros))[:5]]
        #     else : 
        #         selected_indices = indices_zeros[torch.randperm(len(indices_zeros))[:len(indices_ones)]]
        #     combined_indices = torch.cat((selected_indices, indices_ones))
        # else:
        #     if len(indices_zeros) == 0 :
        #         selected_indices = indices_ones[torch.randperm(len(indices_ones))[:5]]
        #     else : 
        #         selected_indices = indices_ones[torch.randperm(len(indices_ones))[:len(indices_zeros)]]
        #     combined_indices = torch.cat((selected_indices, indices_zeros))
        ...
        #         hand_depth,seg, eepose, front_depth,
        #     )
     
        pred_spillage = self.model(
                eepose, ee_pcd, hand_depth, front_depth, hand_pcd, front_pcd, hand_seg, front_seg
            )
        ...
        #     gd_class = spillage[i].item()
        #     if pred_spillage[i].item() < 0.5:
        #         pre_class = 0
        #     else :
        #         pre_class = 1
        #     if gd_class == pre_class :  
